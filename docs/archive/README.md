...
| [`longhorn-evaluation.md`](longhorn-evaluation.md) | The evidence behind choosing Longhorn (replicated block storage) over the alternatives — Postgres migration for the arrs (rejected: unsupported SQLite migration path, breaks the built-in backup) and doing nothing (a legitimate but inferior baseline). Also the reasoning behind the decided order `Longhorn → NAS → Talos`, each step unblocking the next. |
| [`superpowers/specs/2026-08-13-longhorn-deployment-design.md`](superpowers/specs/2026-08-13-longhorn-deployment-design.md) | The design decisions for how Longhorn was deployed and why: three storage nodes with `numberOfReplicas: 2` (right-sizing removed the capacity argument for excluding worker-00), claims right-sized from a 67Gi total to 17Gi, no VPA (no VPA-targetable controller, and `Recreate` would break every attached volume on a restart), and `longhorn` never made the cluster default StorageClass. |
